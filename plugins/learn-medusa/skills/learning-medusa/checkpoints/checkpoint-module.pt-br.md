...
1. **O que o `MedusaService()` faz?**
   ...
   O `MedusaService()` é uma fábrica de serviços fornecida pelo Medusa que gera um serviço com métodos CRUD (criar, atualizar, recuperar, listar, excluir) para o seu modelo de dados. Isso evita que você tenha que escrever código repetitivo.
   </details>
