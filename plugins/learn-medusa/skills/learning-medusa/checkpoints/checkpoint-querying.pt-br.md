...
1. **Por que usamos `+brand.*` no parâmetro fields?**
   ...
   O `+` significa “inclua esses campos ALÉM dos campos padrão”. Sem o `+`, você substituiria os campos padrão por completo. O `.*` significa “inclua todos os campos da relação da marca”. Portanto, `+brand.*` significa “me forneça todos os campos padrão do produto MAIS todos os campos da marca”.
   </details>
...
### “Não é possível ler a propriedade 'result' de um valor indefinido”
...
**Duas formas de consultar dados vinculados**:
