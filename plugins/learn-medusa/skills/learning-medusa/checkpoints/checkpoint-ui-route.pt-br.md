...
2. **Por que usamos `sdk.client.fetch()` em vez de `sdk.admin.brand.list()`?**
   ...
   `sdk.admin.brand.list()` não existe porque a rota da API `/admin/brands` é personalizada, e o SDK JS possui apenas métodos para rotas da API principal. Para rotas de API personalizadas, use `sdk.client.fetch()`, que faz uma solicitação HTTP bruta para qualquer endpoint.
   </details>
...
- [ ] Utiliza o hook `useDataTable()` com colunas, dados, número de linhas e paginação
