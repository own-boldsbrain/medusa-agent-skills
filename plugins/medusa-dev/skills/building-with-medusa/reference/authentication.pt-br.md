...
**Essas rotas não requerem configuração adicional** — a autenticação é feita automaticamente pelo Medusa.
...
**⚠️ CRÍTICO - Validação manual**: NÃO valide manualmente a autenticação em seus manipuladores de rota ao usar o middleware `authenticate`. O middleware já garante que o usuário esteja autenticado — verificações manuais são redundantes e indicam um equívoco sobre como o middleware funciona.
