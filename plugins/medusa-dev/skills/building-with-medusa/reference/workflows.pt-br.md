...
- ❌ Não use ciclos `for`/`while` → Utilize as alternativas abaixo de acordo com o seu caso de uso

As funções de composição de fluxo de trabalho são executadas no momento do carregamento do aplicativo para definir a estrutura do fluxo de trabalho, e não para executar lógica. Não é possível usar ciclos diretamente na função de composição. Em vez disso, utilize estes padrões:

### **Alternativa 1: Ciclo no código de chamada (repetir todo o fluxo de trabalho)**
