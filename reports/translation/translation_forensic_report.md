# Translation Forensic Report

Gerado em: 2026-07-06 13:45:41

## Resumo Global
- **Total de Pares**: 72
- 🟢 **Passed**: 59
- 🟡 **Needs Review**: 13
- 🔴 **Failed**: 0

## Token FinOps
- **Tokens se retraduzir tudo**: 99313
- **Tokens após triagem (High-Token P0)**: 0
- **Tokens economizados (Zero-Token P1/P2)**: 99313

## Ocorrências (Por Arquivo vs Frequência Absoluta)
| Problema | Arquivos Afetados | Total de Ocorrências |
|---|---|---|
| english_residual_detected | 13 | 13 |

## Top 20 Arquivos Mais Críticos (Risco Máximo = 100)
- **Score: 30** | `SKILL.pt-br.md`
  - Ação: `repair_by_script`
  - Issues: english_residual_detected
- **Score: 30** | `design.pt-br.md`
  - Ação: `repair_by_script`
  - Issues: english_residual_detected
- **Score: 30** | `navbar.pt-br.md`
  - Ação: `repair_by_script`
  - Issues: english_residual_detected
- **Score: 30** | `SKILL.pt-br.md`
  - Ação: `repair_by_script`
  - Issues: english_residual_detected
- **Score: 30** | `lesson-1-custom-features.pt-br.md`
  - Ação: `repair_by_script`
  - Issues: english_residual_detected
- **Score: 30** | `lesson-2-extend-medusa.pt-br.md`
  - Ação: `repair_by_script`
  - Issues: english_residual_detected
- **Score: 30** | `SKILL.pt-br.md`
  - Ação: `repair_by_script`
  - Issues: english_residual_detected
- **Score: 30** | `SKILL.pt-br.md`
  - Ação: `repair_by_script`
  - Issues: english_residual_detected
- **Score: 30** | `data-loading.pt-br.md`
  - Ação: `repair_by_script`
  - Issues: english_residual_detected
- **Score: 30** | `display-patterns.pt-br.md`
  - Ação: `repair_by_script`
  - Issues: english_residual_detected
- **Score: 30** | `table-selection.pt-br.md`
  - Ação: `repair_by_script`
  - Issues: english_residual_detected
- **Score: 30** | `SKILL.pt-br.md`
  - Ação: `repair_by_script`
  - Issues: english_residual_detected
- **Score: 30** | `medusa-exec.pt-br.md`
  - Ação: `repair_by_script`
  - Issues: english_residual_detected
- **Score: 0** | `README.pt-br.md`
  - Ação: `none`
  - Issues:
- **Score: 0** | `README.pt-br.md`
  - Ação: `none`
  - Issues:
- **Score: 0** | `SKILL.pt-br.md`
  - Ação: `none`
  - Issues:
- **Score: 0** | `template.pt-br.md`
  - Ação: `none`
  - Issues:
- **Score: 0** | `schema.pt-br.md`
  - Ação: `none`
  - Issues:
- **Score: 0** | `README.pt-br.md`
  - Ação: `none`
  - Issues:
- **Score: 0** | `SKILL.pt-br.md`
  - Ação: `none`
  - Issues:
